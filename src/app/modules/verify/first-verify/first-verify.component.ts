...
@Component({
  selector: 'app-first-verify',
  templateUrl: './first-verify.component.html',
  styleUrls: ['./first-verify.component.scss'],
})
export class FirstVerifyComponent implements OnInit {
  ...
  public changePicture1(event) {
    ...
    if (this.imageToUpload1) {
      ...
      reader.onload = (event: any) => {
        ...
        this.uploadedImage1 = this.sanitizer.sanitize(
  ...
  public changePicture2(event) {
    ...
    if (this.imageToUpload2) {
      ...
      reader.onload = (event: any) => {
        ...
        this.uploadedImage2 = this.sanitizer.sanitize(
          SecurityContext.RESOURCE_URL,
  ...
  public changePicture3(event) {
    ...
    if (this.imageToUpload3) {
      ...
      reader.onload = (event: any) => {
        ...
        this.uploadedImage3 = this.sanitizer.sanitize(
          SecurityContext.RESOURCE_URL,
  ...
  private getUpload() {
    this.dashboard.getDoc().subscribe((data: any[]) => {
      this.uploads = data;
      this.spinner.hide();
    });
  }
  ...
  onDrop() {
    ...
    formData.append('driver_licence', this.imageToUpload3);
    formData.append('ssn', this.nuncracker);
    this.dashboard.uploadDoc(formData).subscribe((res) => {
