...
export class PersonalInfoComponent implements OnInit {
    ...
    this.dashboard.getAccount().subscribe((data: any[]) => {
      ...
      if (this.data) {
        this.form.patchValue({
          ...
          dob: this.data['0'].owner.dob ? this.data['0'].owner.dob : '',
          //  citizenship: this.data['0'].citizenship ?  this.data['0'].citizenship : '',

          country: this.data['0']?.owner?.country,
          city: this.data['0']?.owner?.city ? this.data['0']?.owner?.city : '',
  ...
  submitForm() {
    this.spinner.show();
    const { phone_number, citizenship, city, country, address, zip_code } =
      this.form.value;
    ...
    this.dashboard.updateAccount(this.id, data).subscribe((res) => {
